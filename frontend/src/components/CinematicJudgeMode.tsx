...
export const CinematicJudgeMode: React.FC<CinematicJudgeModeProps> = ({
  ...
  // Runtime figures shown in this presentation are read from the running system.
  // Several were previously asserted as constants - "5.8s TOTAL" for a pipeline
  // that takes far longer with real speech synthesis, and an identity threshold
  // that did not match the configured one. A trailer that contradicts its own
  // deployment is worse than one that says "measuring".
    ...
    (async () => {
      try {
        const [healthRes, mcpRes] = await Promise.all([
          fetch('/health'),
          fetch('/api/mcp/trace/verify?limit=1'),
        ]);
        ...
        setLive({
          identityThreshold: health?.services?.guardian?.identity_threshold,
          mcpLatencyMs: mcp?.verification_call?.latency_ms,
          mcpServer: mcp?.verification_call?.server_identity,
          alpha: 0.05,
        });
      } catch {
        // Leave figures as "measuring" rather than substituting invented ones.
      }
    })();
  ...
  const fmtMs = (v?: number) => (v === undefined || v === null ? 'measuring…' : `${v.toFixed(1)}ms`);
  ...
  const chapters = useMemo(() => [
    {
      ...
      beats: [
        ...
        },
        {
          ...
          stat: live?.identityThreshold ? `COSINE >= ${live.identityThreshold}` : "COSINE THRESHOLD",
          statLabel: "Configured Identity Authentication Threshold"
        }
      ],
      interactiveAction: {
    ...
    },
    {
      ...
      beats: [
        ...
        },
        {
          ...
          stat: "9-STAGE DAG",
          statLabel: "Research -> Script -> Critic -> Director -> Render -> Guardian -> Gate"
        }
      ],
      interactiveAction: {
      ...
      beats: [
        {
          ...
          highlight: "ASR + Gemini first-token latency + neural audio, budgeted under the 800ms design target",
          ...
          stat: "<800ms",
          statLabel: "End-to-End Latency Design Target"
        },
        {
          ...
          stat: "BARGE-IN",
          statLabel: "Interrupt Cancels Playback Mid-Utterance"
        }
      ],
      interactiveAction: {
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: '10px', height: '100%', justifyContent: 'space-between' }}>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: '8px', flex: 1, justifyContent: 'center' }}>
            {[
              ...
              { step: "Gemini first-token", ms: 240, pct: "50%", color: "#B06000" },
      ...
      beats: [
        {
          ...
          stat: fmtMs(live?.mcpLatencyMs),
          statLabel: live?.mcpServer
            ? `Measured now via ${live.mcpServer}`
            : "ClickHouse Query Latency via the Official MCP Server"
        },
        {
          ...
          stat: live?.alpha ? `p < ${live.alpha}` : "WELCH T-TEST",
          statLabel: "Significance Required Before a Strategy Changes"
        }
      ],
      interactiveAction: {
